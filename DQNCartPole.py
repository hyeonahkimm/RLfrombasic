...
class ReplayBuffer():
    ...
    def sample(self, n):
        ...
        for transition in mini_batch:
            ...
            done_mask_lst.append([done_mask])  # done_mask: 0 if done, 1 o/w
...
class Qnet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(4, 128)  # state = (cart_pos, cart_vel, pole_degree, pole_acc)
        self.fc2 = nn.Linear(128, 128)
        self.fc3 = nn.Linear(128, 2)  # action 2 (left or right)
...
def train(q, q_target, memory, optimizer):
    for i in range(10):
        ...
        q_out = q(s)  # forward?
        q_a = q_out.gather(1, a)  # gather(dim, index) # Q(s,a)
...
def main():
    ...
    for n_epi in range(10000):
        epsilon = max(0.01, 0.08 - 0.01*(n_epi/200))  # Linear annealing from 0.08 to 0.01 (every 200 epi)
        s = env.reset()
        done = False
        ...
        while not done:
            ...
            memory.put((s, a, r/100.0, s_prime, done_mask))  # reward scaling
            s = s_prime
            score += r
            if done:
                break
